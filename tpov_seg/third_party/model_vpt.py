...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompt_length=0, prompt_depth=0):
        ...
        self.prompt_tokens = nn.Parameter(torch.zeros(prompt_depth, prompt_length, width,dtype=torch.float16)) if prompt_length > 0 else None
    ...
    def forward(self, x: torch.Tensor, dense=False, prompt=None):
        ...
        for i, resblock in enumerate(self.resblocks):
            ...
            # # 如果使用prompt且当前层小于prompt_depth
            if self.prompt_length > 0 and i < self.prompt_depth:
            # 插入prompt tokens
            # x[0:1,:,:] - 保留第一个token([CLS])
            # self.prompt_tokens[i].repeat(x.shape[1], 1, 1).permute(1, 0, 2) - 将当前层的prompt扩展到batch size并调整维度顺序
            # x[1:,:,:] - 原始序列的剩余tokens
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        #添加对于coop的嵌入向量初始化    这里每次跑之前要修改
        # self.coop_tokens= nn.Parameter(torch.zeros(7, prompt_length, transformer_width,dtype=torch.float16)) if prompt_length > 0 else None         #这里的width是特征维度
        # if self.coop_tokens is not None:
        #     nn.init.xavier_uniform_(self.coop_tokens)
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisualTransformer(
                ...
                heads=vision_heads,
                output_dim=embed_dim,
                prompt_depth=0,        # if not text_prompt else 0,
                prompt_length=0,      # if not text_prompt else 0,
            )
        ...
        self.transformer = Transformer(
            ...
            prompt_depth=prompt_depth,      #prompt_depth,               
            prompt_length=prompt_length,     #prompt_length
            # deep_replace_method=deep_replace_method,
        )

        self.vocab_size = vocab_size
        self.token_embedding = nn.Embedding(vocab_size, transformer_width)
        self.positional_embedding = nn.Parameter(torch.empty(self.context_length, transformer_width))    #是一个预先定义好的位置编码参数矩阵
    ...
    def build_attention_mask(self):
        # 原始长度 + prompt长度
        # total_length = self.context_length + self.prompt_length if hasattr(self, 'prompt_length') else self.context_length       #如果是coop的话2*prompt修改为对应两倍
        total_length = self.context_length+self.prompt_length                                                                                         #这里改了要记得改回来
    ...
    def encode_text(self, text, prompt=None):
        #if prompt is not None:
        #import pdb; pdb.set_trace()                                                          #  torch.Size([7*6, 77])  
        x = self.token_embedding(text).type(self.dtype)                                       #  torch.Size([7*6, 77, 512])    对42个序列分别处理       
        if prompt is not None:
            x[:, 1:5] = prompt
        x = x + self.positional_embedding.type(self.dtype)  

        # if self.prompt_length>0:
        #     cls_token = x[:, 0:1, :]                                    # [42, 1, 512]
        #     remain_tokens = x[:, 1:, :]                                 # [42, 76, 512] 
        #     dim = x.shape[2]
        #     x = torch.cat([cls_token, self.coop_tokens, remain_tokens], dim=1)                       # 理论上[42，4，512]
        #     #这个地方的位置编码需要做对应更新    
        #     positional_embedding = nn.Parameter( torch.empty(self.context_length+self.prompt_length, dim, device='cuda:0'))
...
################################################################
    ...
    new_state_dict = OrderedDict()                                              #一种特殊的字典类型，它会记住字典中键值对的插入顺序
    for k, v in state_dict.items():                         
        if "in_proj_weight" in k:
            _q, _k, _v = v.chunk(3, dim=0)                                      #将这个合并的权重矩阵用 chunk(3, dim=0) 分成三等份,使用分离的 Q、K、V 权重矩阵
