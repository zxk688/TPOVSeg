...
from tpov_seg.third_party import clip
from tpov_seg.third_party import imagenet_templates
import cv2
...
class CATSegPredictor(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.prompt_ensemble_type == "TTL":
        ...
        if self.prompt_ensemble_type == "TTL":
            prompt_channel=len(prompt_templates)*3                                      #number of words
        elif self.prompt_ensemble_type == "single":  
    ...
    @staticmethod
    def process_tokens(tokens, clip_model, prompt=None):

        if len(tokens.shape) == 3:  
            num_classes, num_templates, seq_len = tokens.shape
            tokens = tokens.reshape(-1, seq_len)  
            
            embeddings = clip_model.encode_text(tokens, prompt)  
            ...
            embeddings = embeddings.unsqueeze(1)  # [num_classes, 1, feature_dim]
    ...
    def get_text_embeds(self, classnames, templates, clip_model, prompt=None):
        ...
        if self.tokens is None or prompt is not None:                                              
            tokens = []
            for classname in classnames:#171个类都进行tokenize
                if ', ' in classname:
                    classname_splits = classname.split(', ')
                    if self.prompt_ensemble_type == "TTL":
                        texts = [template.format(word) for template in templates for word in classname_splits]           
                    elif self.prompt_ensemble_type == "single":  
        ...
        class_embeddings = self.process_tokens(tokens, clip_model, prompt)                                   
