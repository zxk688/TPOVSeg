...
class SwinTransformerBlockWrapper(nn.Module):
    ...
    def forward(self, x, appearance_guidance):
        ...
        x = self.block_1(x, appearance_guidance)        # 局部窗口内具有自注意力特征
        x = self.block_2(x, appearance_guidance)        # 移位窗口内具有自注意力特征
...
class ClassTransformerLayer(nn.Module):
    ...
    def forward(self, x, guidance):
        ...
        """
        B, C, T, H, W = x.size()
        x_pool = self.pool_features(x)  # torch.Size([1, 128, 7, 24, 24])   ->  [1, 128, 7, 12, 12]
        *_, H_pool, W_pool = x_pool.size()
        ...
        x_pool = rearrange(x_pool, 'B C T H W -> (B H W) T C')  # [1, 128, 7, 12, 12]   ->  [144, 256, 128]
        if guidance is not None:    # torch.Size([1, 7, 128])   ->  torch.Size([144, 256, 128])
        ...
        if self.padding_tokens is not None:     # torch.Size([1, 128, 256, 24, 24])     ->      torch.Size([1, 128, 7, 24, 24])
            if orig_len < self.pad_len:
...
class AggregatorLayer(nn.Module):
    def __init__(self, hidden_dim=64, text_guidance_dim=512, appearance_guidance=512, nheads=4, input_resolution=(20, 20), pooling_size=(5, 5), window_size=(10, 10), attention_type='linear', pad_len=256) -> None:
        ...
        self.channel_attention = TGCA(hidden_dim=hidden_dim, guidance_dim=text_guidance_dim, nheads=nheads, num_blocks=2, pooling_size=pooling_size, pad_len=pad_len)
...
# Cost Aggregation Model for CAT-Seg(用于处理图像特征和文本特征之间的关联)
class Aggregator(nn.Module):    
    def __init__(self, 
        text_guidance_dim=512,                  # 文本指导的维度
        text_guidance_proj_dim=128,             # 投影后的文本指导维度
        appearance_guidance_dim=512,            # 外观指导的维度
        appearance_guidance_proj_dim=128,       # 投影后的外观指导维度
        decoder_dims = (64, 32),            
        decoder_guidance_dims=(256, 128),       # 解码器指导的维度
        decoder_guidance_proj_dims=(32, 16),    # 解码器指导的投影维度
        num_layers=4,                           # 聚合器的层数
        nheads=4,                               # 注意力头的数量
        hidden_dim=128,                         # 隐藏层的维度，默认为128。用于定义Transformer块的隐藏维度
        pooling_size=(6, 6),                    # 聚合层的池化大小
        feature_resolution=(24, 24),            # 特征分辨率
        window_size=12,                         # Swin块的窗口大小
        attention_type='linear',                # 注意力类型
        prompt_channel=1,                       # 进行文本特征集成的提示数量
        pad_len=256,                            # 类聚合的填充长度，默认为256
    ) -> None:
        """
    ...
    def forward(self, img_feats, text_feats, appearance_guidance):
        """
        Arguments:  
            img_feats: (B, C, H, W)          [1, 512, 24, 24]
            text_feats: (B, T, P, C)         [1, 7, 6, 512]
            apperance_guidance: tuple of (B, C, H, W)
        """
        classes = None

        corr = self.correlation(img_feats, text_feats)      # img_feats: torch.Size([1, 512, 24, 24])     text_feats: torch.Size([1, 7, 6, 512])    coor: torch.Size([1, 1, 7, 24, 24])
        if self.pad_len > 0 and text_feats.size(1) > self.pad_len:      # if没进去
        ...
        corr_embed = self.corr_embed(corr)                  # corr_embed: torch.Size([1, 128, 7, 24, 24])

        projected_guidance, projected_text_guidance, projected_decoder_guidance = None, None, [None, None]
        if self.guidance_projection is not None:                                        # pre: [1, 512, 24, 24]    proj: [1, 128, 24, 24]
            projected_guidance = self.guidance_projection(appearance_guidance[0])   
        if self.decoder_guidance_projection is not None:                                # pre: [1, 256, 48, 48],[1, 128, 96, 96]   proj: [1, 32, 48, 48],[1, 16, 96, 96]
            projected_decoder_guidance = [proj(g) for proj, g in zip(self.decoder_guidance_projection, appearance_guidance[1:])]    

        if self.text_guidance_projection is not None:                                   # pre: [1, 7, 1, 512]    proj: [1, 7, 128]
            text_feats = text_feats.mean(dim=-2)                                        # 模板提示特征平均  [1, 7, 6, 512] --->[1, 7,  512] 
            text_feats = text_feats / text_feats.norm(dim=-1, keepdim=True)           
            projected_text_guidance = self.text_guidance_projection(text_feats)         # 降维([1, 7, 128])
...
class ChannelAttention(nn.Module):
    """多头通道注意力机制"""
    def __init__(self, channels, guidance_dim=0, num_heads=8, head_dim=None, reduction=16, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.channels = channels
        self.guidance_dim = guidance_dim
        self.num_heads = num_heads
        head_dim = head_dim or max(channels // num_heads, 8)  # 确保head_dim至少为8
        attn_dim = head_dim * num_heads
        ...
        # Query, Key, Value for channel attention
        input_dim = channels + guidance_dim
        self.q = nn.Linear(input_dim, attn_dim, bias=True)
        self.k = nn.Linear(input_dim, attn_dim, bias=True)
        self.v = nn.Linear(channels, attn_dim, bias=True)
        ...
        self.channel_pos_embed = nn.Parameter(torch.zeros(1, channels))
    ...
    def forward(self, x, guidance=None):
        ...
        Args:
            x: (B*T, C, H, W)
            guidance: (B*T, guidance_dim) 
        """
        B_, C, H, W = x.shape
        
        # Extract channel-wise global features
        global_feat = self.global_pool(x).view(B_, C)  # B_, C
        max_feat = self.max_pool(x).view(B_, C)  # B_, C
        channel_feat = (global_feat + max_feat) / 2 + self.channel_pos_embed
        
        # Concatenate with guidance if available
        if guidance is not None and self.guidance_dim > 0:
            # 简化guidance处理，直接广播到通道维度
            if guidance.dim() == 2:  # B_, guidance_dim
                guidance_feat = guidance.unsqueeze(1).expand(-1, C, -1)  # B_, C, guidance_dim
                guidance_feat = guidance_feat.mean(dim=-1)  # B_, C 
                channel_feat_with_guidance = torch.cat([channel_feat, guidance_feat], dim=-1)
            else:
                channel_feat_with_guidance = channel_feat
        else:
            channel_feat_with_guidance = channel_feat
        
        # Multi-head attention for channels - 简化版本
        q = self.q(channel_feat_with_guidance)  # B_, attn_dim
        k = self.k(channel_feat_with_guidance)  # B_, attn_dim  
        v = self.v(channel_feat)  # B_, attn_dim
        
        # Reshape for multi-head attention
        q = q.view(B_, self.num_heads, self.head_dim)  # B_, num_heads, head_dim
        k = k.view(B_, self.num_heads, self.head_dim)  # B_, num_heads, head_dim
        v = v.view(B_, self.num_heads, self.head_dim)  # B_, num_heads, head_dim
        
        q = q * self.scale
        attn = torch.sum(q * k, dim=-1, keepdim=True)  # B_, num_heads, 1
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)
        
        x_attn = (attn * v).sum(dim=1)  # B_, head_dim
        x_attn = x_attn.repeat(1, self.channels // self.head_dim)  # B_, channels
        x_attn = self.proj_drop(x_attn)
        
        # Apply channel attention weights
        channel_weights = torch.sigmoid(x_attn).view(B_, C, 1, 1)  # B_, C, 1, 1
        return x * channel_weights


class ChannelMLP(nn.Module):
    """通道维度的MLP"""
    def __init__(self, channels, mlp_ratio=4., act_layer=nn.GELU, drop=0.):
        super().__init__()
...
class ChannelTransformerBlock(nn.Module):
    """通道Transformer块"""
    def __init__(self, channels, guidance_dim=0, num_heads=8, head_dim=None, mlp_ratio=4., 
        ...
        self.attn = ChannelAttention(
            channels, guidance_dim=guidance_dim, num_heads=num_heads, head_dim=head_dim,
            attn_drop=attn_drop, proj_drop=drop
        )
        
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(channels)
        self.mlp = ChannelMLP(channels, mlp_ratio=mlp_ratio, act_layer=act_layer, drop=drop)
        
        # Channel interaction convolution
        self.channel_conv = nn.Conv1d(channels, channels, kernel_size=3, padding=1, groups=channels//8)
        self.channel_norm = nn.BatchNorm1d(channels)
        
    def forward(self, x, guidance=None):
        """
        Args:
            x: (B*T, C, H, W)                 torch.Size([1280, 128, 24, 24])
            guidance: (B*T, guidance_dim)     torch.Size([1280, 128])
        """
        B_, C, H, W = x.shape
        
        # Channel-wise global pooling for transformer processing
        channel_feat = x.mean(dim=(2, 3))  # B_, C    torch.Size([1280, 128])
        
        # Channel attention with residual connection
        shortcut = channel_feat
        channel_feat = self.norm1(channel_feat)
        x_attended = self.attn(x, guidance)  # Apply attention to full feature map
        
        # Extract attended channel features
        attended_channel_feat = x_attended.mean(dim=(2, 3))  # B_, C               #torch.Size([1280, 128, 24, 24])到
        channel_feat = shortcut + self.drop_path(attended_channel_feat)
        
        # Channel interaction through 1D convolution
        channel_feat_conv = self.channel_conv(channel_feat.unsqueeze(-1)).squeeze(-1)
        channel_feat_conv = self.channel_norm(channel_feat_conv)
        channel_feat = channel_feat + F.relu(channel_feat_conv)
        
        # Channel MLP with residual connection
        channel_feat = channel_feat + self.drop_path(self.mlp(self.norm2(channel_feat)))
        
        # Apply final channel weights to feature map
        final_weights = torch.sigmoid(channel_feat).unsqueeze(-1).unsqueeze(-1)  # B_, C, 1, 1
        return x + x * final_weights



class TGCA(nn.Module):
    def __init__(self, hidden_dim=64, guidance_dim=64, nheads=8, num_blocks=2, 
                 mlp_ratio=4., pooling_size=(4, 4), pad_len=256, 
                 drop=0., attn_drop=0., drop_path=0.1) -> None:
        super().__init__()
        
        # Multi-scale channel processing
        self.pool = nn.AvgPool2d(pooling_size) if pooling_size is not None else nn.Identity()
        
        # Multiple channel transformer blocks
        self.blocks = nn.ModuleList([
            ChannelTransformerBlock(
                ...
                drop=drop,
                attn_drop=attn_drop,
                drop_path=drop_path * (i / max(1, num_blocks - 1))  # Stochastic depth
        ...
        # Cross-temporal channel aggregation
        self.temporal_channel_attn = AttentionLayer(hidden_dim, guidance_dim, nheads=nheads, attention_type='linear')
        
        # Channel enhancement MLP
        ...
        # Channel position embedding for temporal modeling
        self.channel_temp_embed = nn.Parameter(torch.zeros(1, pad_len if pad_len > 0 else 256, hidden_dim))
        
    def pool_features(self, x):
        """
        Multi-scale pooling for computational efficiency
        Arguments:
            x: B, C, T, H, W
        """
        B = x.size(0)
    ...
    def forward(self, x, guidance):
        """
        Arguments:
            x: B, C, T, H, W    5,128,2,24,24
            guidance: B, T, C   5,2,128
        """
        B, C, T, H, W = x.size()
        
        # Multi-scale pooling
        x_pool = self.pool_features(x)
        *_, H_pool, W_pool = x_pool.size()   #torch.Size([5, 128, 2, 24, 24])
        
        # Handle padding for consistent sequence length
        orig_len = T
        if self.padding_tokens is not None and orig_len < self.pad_len:
            padding_tokens = repeat(self.padding_tokens, '1 1 C -> B C T H W', 
                                  B=B, T=self.pad_len - orig_len, H=H_pool, W=W_pool)
            x_pool = torch.cat([x_pool, padding_tokens], dim=2)
            T = self.pad_len
            
        if guidance is not None and self.padding_guidance is not None and orig_len < self.pad_len:
            padding_guidance = repeat(self.padding_guidance, '1 1 C -> B T C', 
                                    B=B, T=self.pad_len - orig_len)
            guidance = torch.cat([guidance, padding_guidance], dim=1)
        
        # Process through channel transformer blocks   通道注意力对（batch ×类别）分别计算
        x_processed = rearrange(x_pool, 'B C T H W -> (B T) C H W')
        if guidance is not None:
            guidance_processed = rearrange(guidance, 'B T C -> (B T) C')
        else:
            guidance_processed = None
            
        for block in self.blocks:
            x_processed = block(x_processed, guidance_processed)
        
        x_processed = rearrange(x_processed, '(B T) C H W -> B C T H W', B=B, T=T)
        
        # Cross-temporal channel aggregation
        x_temp = rearrange(x_processed, 'B C T H W -> (B H W) T C')
        
        # Add temporal position embedding
        if x_temp.size(1) <= self.channel_temp_embed.size(1):
            temp_embed = self.channel_temp_embed[:, :x_temp.size(1), :]
            x_temp = x_temp + temp_embed
        
        if guidance is not None:
            guidance_temp = repeat(guidance, 'B T C -> (B H W) T C', H=H_pool, W=W_pool)
        else:
            guidance_temp = None

        #第二阶段类别聚合    
        x_temp = x_temp + self.temporal_channel_attn(self.norm(x_temp), guidance_temp)
        x_temp = x_temp + self.channel_enhance(self.norm(x_temp))
        
        # Reshape back and interpolate to original size
        x_temp = rearrange(x_temp, '(B H W) T C -> (B T) C H W', H=H_pool, W=W_pool)
        x_temp = F.interpolate(x_temp, size=(H, W), mode='bilinear', align_corners=True)
        x_temp = rearrange(x_temp, '(B T) C H W -> B C T H W', B=B)
        
        # Remove padding if added  去掉填充
        if self.padding_tokens is not None and orig_len < self.pad_len:
            x_temp = x_temp[:, :, :orig_len]
