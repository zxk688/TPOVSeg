...
def debug_targets_assignment(targets, mask, onehot, line_number=183):
    """
    调试_targets[mask] = _onehot的索引越界问题
    
    在cat_seg_model.py第183行之前调用这个函数
    """
    print(f"\n=== 调试第{line_number}行: _targets[mask] = _onehot ===")
    ...
    print(f"onehot.dtype: {onehot.dtype}")
    print(f"onehot min/max: {onehot.min()}/{onehot.max()}")
    
    # 检查索引范围
    if len(targets.shape) >= 2:
        print(f"targets允许的索引范围: 0-{targets.shape[0]-1}, 0-{targets.shape[1]-1}")
    
    # 检查mask中的值是否超出targets的索引范围
    mask_indices = torch.where(mask)
    if len(mask_indices) > 0:
        for i, indices in enumerate(mask_indices):
            if len(indices) > 0:
                print(f"mask维度{i}的索引范围: {indices.min()}-{indices.max()}")
                if i < len(targets.shape):
                    if indices.max() >= targets.shape[i]:
                        print(f"⚠️  维度{i}索引越界: max={indices.max()} >= shape={targets.shape[i]}")
    
    # 检查onehot是否有超出范围的值
    if len(onehot.shape) > 0:
        onehot_max = onehot.max()
        if len(targets.shape) > 2 and onehot_max >= targets.shape[-1]:
            print(f"⚠️  onehot值超出范围: max={onehot_max} >= num_classes={targets.shape[-1]}")
...
class CATSeg(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        clip_images.tensor = clip_images.tensor.requires_grad_(True)
        

        was_training = self.sem_seg_head.predictor.clip_model.training
        self.sem_seg_head.predictor.clip_model.train()
        ...
        self.sem_seg_head.predictor.clip_model.train(was_training)
