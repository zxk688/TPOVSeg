...
GLHWATER_CATEGORIES = [
    ...
    {'color': [255, 255, 255], 'isthing': 1, 'id': 1, 'name': 'water', 'trainId': 1},
]

def _get_loveda_meta():
    stuff_ids = [(k["id"]) for k in GLHWATER_CATEGORIES]
    assert len(stuff_ids) == 2, len(stuff_ids)
    stuff_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(stuff_ids)}
    stuff_classes = [k["name"] for k in GLHWATER_CATEGORIES]
...
def register_all_potsdam(root):
    ...
    for name, image_dirname, sem_seg_dirname in [
        ...
        name = f"glhwater_{name}_sem_seg"
